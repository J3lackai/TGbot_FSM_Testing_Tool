...
4.  **(Настоятельно Рекомендуется) Аккаунт и учетные данные Тестового Сервера:** Для более безопасного и менее ограниченного тестирования крайне рекомендуется использовать тестовые серверы Telegram. Это требует:
    *   Доступа к тестовому окружению (см. **Использование Тестового Сервера** ниже).
    *   Номера телефона, зарегистрированного *на тестовом сервере*. Возможно, подойдет ваш обычный номер, если он там еще не зарегистрирован, или может понадобиться отдельный номер.
    *   *Отдельных* `api_id` и `api_hash`, полученных при регистрации приложения *в тестовом окружении Telegram*. **Не используйте API-ключи от основного ("production") сервера!**

## Начало работы

1.  **Скачайте последний релиз:**
    *   Перейдите на [страницу релизов]([[СЮДА_ВСТАВЬТЕ_ССЫЛКУ_НА_РЕЛИЗЫ_ВАШЕГО_РЕПОЗИТОРИЯ](https://github.com/J3lackai/TGbot_FSM_Testing_Tool/releases/tag/Python)]) этого репозитория.
...
2.  **Настройте Ключ Шифрования (Переменная Окружения):**
    ...
    *   **Если эта переменная не установлена:** При первом запуске скрипт сгенерирует *временный* ключ и выведет **предупреждение** с самим ключом. Вы **ОБЯЗАТЕЛЬНО** должны установить этот сгенерированный ключ как постоянную переменную окружения (`ENCRYPTION_KEY`) на вашем компьютере, чтобы зашифрованная сессия работала при последующих запусках. В противном случае вам придется проходить авторизацию каждый раз, и предыдущая зашифрованная сессия станет недоступна.
...
3.  **Создайте Файлы Тестов (`.txt`):**
    ...
    *   Обратитесь к файлу `sample_test.txt` ([СЮДА_ВСТАВЬТЕ_ССЫЛКУ_ИЛИ_УКАЖИТЕ_ПУТЬ_К_ПРИМЕРУ]) для ознакомления с требуемым форматом. Каждая непустая строка должна соответствовать шаблону:
        ```
...
2.  **Используйте тестовые учетные данные:** Убедитесь, что в вашем файле `.env` указаны `TELEGRAM_API_ID`, `TELEGRAM_API_HASH` и `PHONE`, полученные **для тестового сервера**.
