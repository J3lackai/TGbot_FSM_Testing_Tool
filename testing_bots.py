...
def testing_bots():
    ...
    if count <= 0:
        ...
        raise IndexError("Ошибка: номер должен быть больше или равен 1.")

    if count > len(tuple_bots):
        logger.critical("Ошибка: бота с таким номером нет в конфиге.")
        raise IndexError("Ошибка: бота с таким номером нет в конфиге.")
